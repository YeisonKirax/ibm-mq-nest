...
import {ClosePromise} from '@yeisonkirax/ibm-mq';
...
export class IbmMqNestModule
  extends createConfigurableDynamicRootModule<IbmMqNestModule, IMQConfig>(
    IBM_MQ_CONFIG_TOKEN,
    {
      providers: [
        {
          ...
          useFactory: async (
            config: IMQConfig,
            conn: IbmMqConnectionFactory,
          ) => {
            const con: IbmMqConnection = await conn.createConnection(
              config.channel,
              config.credentials,
              config.options,
            );
  ...
  async onApplicationShutdown(signal?: string) {
    ...
    connections.forEach((connection) => {
      const objectNamesGetMessages = Object.keys(
        connection.objectsOpenedToGetMessages,
      );
      for (const queueName of objectNamesGetMessages) {
        closeQueuesOpenedPromises.push(
          ClosePromise(connection.objectsOpenedToGetMessages[queueName], 0),
        );
      }
    });
    ...
    await Promise.all(
      this.connectionManager
        .getConnections()
        .map((connection) => connection.closeConnection()),
    );
  ...
  async onApplicationBootstrap() {
    ...
    let mqMetas =
      await this.discover.providerMethodsWithMetaAtKey<IMQDecoratorOptions>(
        IBM_MQ_HANDLER,
      );
    mqMetas = mqMetas.concat(
      await this.discover.controllerMethodsWithMetaAtKey<IMQDecoratorOptions>(
        IBM_MQ_HANDLER,
      ),
    );
    for (const connection of connections) {
      const metasUsingThisConnection = mqMetas;
      const mqMetasGrouped = groupBy(
        metasUsingThisConnection,
      ...
      for (const providerKey of providerKeys) {
        ...
        await Promise.all(
          mqMetasGrouped[providerKey].map(
            async ({discoveredMethod, meta}) => {
              ...
              return meta.targetType === MQObjectType.TOPIC
                ? connection.createTopicSubscriber(
                  handler,
                  mergedConfig,
                  discoveredMethod.methodName,
                )
                : connection.createQueueSubscriber(
                  handler,
                  mergedConfig,
                  discoveredMethod.methodName,
                );
